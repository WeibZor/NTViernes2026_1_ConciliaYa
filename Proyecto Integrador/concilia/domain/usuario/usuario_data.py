...
def generar_fecha_nacimiento(min_edad=18, max_edad=90):
    hoy = datetime.now()
    edad = random.randint(min_edad, max_edad)
    nacimiento = hoy - timedelta(days=365 * edad + random.randint(0, 365))
...
def generar_datos_usuario(num_registros=1000, semilla=None):
    ...
    nombres = ["Andrés", "María", "Juan", "Luisa", "Carlos", "Ana", "Pedro", "Sofía", "Diego", "Valeria"]
    apellidos = ["Pérez", "Gómez", "Rodríguez", "García", "Martínez", "López", "Ramírez", "Torres"]
    tipos_documento = ["CC", "TI", "CE", "PAS"]

    filas = []
    for i in range(1, num_registros + 1):
        nombre = random.choice(nombres)
        apellido = random.choice(apellidos)
        documento = f"{random.randint(1000000, 99999999)}"
...
def validar_recarga_usuario(df_origen, df_csv, df_json):
    ...
    claves = ["Id", "Nombres", "Documento", "Activo"]
    for col in claves:
...
def main():
    ...
    exportar_datos_usuario(df)
    df_csv, df_json = cargar_datos_usuario()
