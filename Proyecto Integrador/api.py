...
CONFLICTO_COLUMN_MAP = {
    "Id": "id",
    "Titulo": "titulo",
    "Descripcion": "descripcion",
    "Tipo Conflicto": "tipoConflicto",
    "Estado": "estado",
    "Activo": "activo",
    "FechaAlta": "fechaAlta",
    "UsuarioId": "usuarioId",
...
def serialize_df(df: pd.DataFrame, column_map: dict = None):
    ...
    if column_map is None:
        column_map = {col: col for col in df_serial.columns}
...
def initialize_conflictos():
    global CONFLICTO_STATE
    if CONFLICTO_STATE is None:
        df = limpiar_conflictos(generar_datos_conflicto(num_registros=1000, semilla=42))
...
def get_conflictos_df():
    return pd.DataFrame(initialize_conflictos())
...
@app.get("/api/conflictos/raw")
def conflictos_raw():
    df = generar_datos_conflicto(num_registros=1000, semilla=42)
    return serialize_df(df, CONFLICTO_COLUMN_MAP)


@app.get("/api/conflictos")
def conflictos_limpios():
    return initialize_conflictos()


@app.get("/api/conflictos/head-tail")
def conflictos_head_tail():
    df = get_conflictos_df()
    return {
        "head": df.head(5).to_dict(orient="records"),
        "tail": df.tail(5).to_dict(orient="records"),
    }
