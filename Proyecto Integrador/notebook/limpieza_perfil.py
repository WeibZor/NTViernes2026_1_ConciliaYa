...
# Campos del PerfilDto: id, nombre, descripcion, activo
# NOTA: el DTO NO incluye fechaAlta (solo el modelo la tiene internamente).

def limpiar_perfil(df_sucio):
    df = df_sucio.copy()

    # 1. Textos
    df["nombre"]      = df["nombre"].astype("string").str.strip().str.lower()
    df["descripcion"] = df["descripcion"].astype("string").str.strip()

    # 2. Valores esperados
    df["nombre"] = df["nombre"].where(
        df["nombre"].isin(["administrador", "gestor", "usuario"]), pd.NA
    )
